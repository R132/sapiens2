...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Run pointmap (3D) estimation on a directory of images'
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Output directory (default: {input}/sapiens_pointmap)'
    )
...
def main():
    ...
    # Navigate to the script's parent directory and go up two levels
    ...
    if args.output is None:
        args.output = os.path.join(args.input, 'sapiens_pointmap')
    ...
    checkpoint = os.path.join(
        ...
        'pointmap',
        f'{model_name}_pointmap.safetensors'
    )

    model_str = f'{model_name}_pointmap_{args.dataset}-1024x768'
    config_file = f'configs/pointmap/{args.dataset}/{model_str}.py'
    # Use absolute path for run_file since we changed directory
    run_file = os.path.join(sapiens_root, 'tools/vis/vis_pointmap.py')

    # Validate paths
    if not os.path.exists(config_file):
        print(f"Warning: Config file not found at {config_file}")
        print(f"  Expected: {os.path.abspath(config_file)}")

    if not os.path.exists(run_file):
        print(f"Error: vis_pointmap.py not found at {run_file}")
        print("Make sure this script is placed in the correct location.")
        print(f"Expected location: {os.path.join(sapiens_root, 'tools/vis/vis_pointmap.py')}")
        sys.exit(1)
    ...
    for i, batch in enumerate(image_batches):
        ...
        # Build command - note: pointmap script doesn't use --save_pred
        cmd = [
